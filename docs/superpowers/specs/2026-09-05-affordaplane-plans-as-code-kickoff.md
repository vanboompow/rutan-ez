...
spec scopes that kickoff. It does not authorize starting work — sea-ez's wing change is the gate.
...
threads). See the brief's §4 recommendation: build the Affordaplane over Aerolite 103 or CGS Hawk
Ultra because its structure matches open-ez's existing CadQuery/tube-fabrication assumptions and it
has the shortest credibly-sourced build-hour number in the survey.
...
for the Rotax 277 (65 lb complete with reduction drive), which is out of production — used-market
sourcing only. Closing that weight gap without giving up 25+ hp means favoring the lightest engine
class the brief surveys: Moster-class paramotor 2-strokes (Vittorazi Moster 185 Plus: 25 hp at
31-34 lb complete, Polini Thor 250: comparable class at ~42 lb) run 30-35 lb lighter than a Rotax
277 for similar or better power. Neither is a factory-drawn Affordaplane engine — this is this
spec's own inference from the brief's engine table, not a documented Affordaplane build — so the
weight roll-up milestone below exists specifically to check whether a Moster-class engine plus the
airframe's real (not advertised) structural weight actually clears 254 lb, before money is spent on
an engine.
...
1. **Fuselage truss as parametric tube+gusset.** Model the bolted-aluminum fuselage truss as a
   ...
   config fields, joint positions derived rather than hard-coded, matching the existing pattern in
   `config/aircraft_config.py`. Output: STEP + DXF (gusset plates as laser-cut parts), mirroring
   open-ez's existing manufacturing-output convention.
2. **Wing rib and spar.** Simpler than Long-EZ's foam-core wing — no airfoil spline smoothing
   needed unless the Affordaplane's wing section requires it (research brief does not specify the
   airfoil; that's an open item for the Facebook builders group ask below). Parametric rib spacing
   and spar cap sizing as a second `AircraftComponent`.
3. **Weight roll-up gated at 254 lb with engine.** A test (following open-ez's existing
   `ComplianceTracker`/pytest pattern) that sums modeled component masses plus a declared engine
   weight and fails if the total exceeds 254 lb. This is the milestone that actually tests the
   Moster-class-engine hypothesis above against modeled (not advertised) airframe weight — it gates
   the design, it doesn't just report a number.

## What must be bought

The Affordaplane plans themselves, from affordaplane.com. The research brief could not find a 2026
plans price in sources retrieved — that's an open item, along with a real builder-reported empty
weight with an engine installed, both of which the brief recommends asking directly in the
Affordaplane Facebook builders group before committing to a spar/tube gauge (brief §5, "A concrete
next step for Ryan").
...
transcribing dimensions, drawings, or text from the plans into config files, comments, or commit
messages. This mirrors the same IP boundary open-ez already respects for the Long-EZ's original
Rutan plans.

## Relation to sea-ez

sea-ez (Part 103 amphibious flying-boat) and this Affordaplane kickoff share a tube/gusset
fabrication library once both exist: bolted-aluminum-tube joint geometry, gusset plate patterns,
and drill-jig generation are the same CadQuery primitives whether the airframe lands on wheels or
water. Building the Affordaplane's simpler truss first is explicitly the de-risking step the
research brief recommends (§5) before sea-ez's hull-adjacent structure needs the same primitives at
higher stakes. No code sharing is proposed in this spec — that's a natural follow-on once both
milestone sets exist to compare.
